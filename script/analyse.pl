...
$opts{p} = $opts{I} ? "$Bin/../config/qiime_ITS_par.txt" : "$Bin/../config/qiime_16S_par.txt"  unless $opts{p};
...
if(@display){
	for my $i( 0 .. $#display ){
		...
		if($pid[$i]==0){
			...
			unless(-s "seqs.fna" ){
				system "perl $Bin/get_seqs_in_fa.pl ../seqs.fna $map 250  1 >seqs.fna";
				system "perl $Bin/reads_stat.pl -m $map -a $opts{g}  seqs.fna"; 
			}
			if($opts{o}){
				&otu($map);
			}
...
sub extract {
	...
	system "perl $Bin/../utils/flash -o out -t $opts{O}  -x  $opts{x} -M 220  @fq";
	system "perl $Bin/reverse_complement_fastq.pl -i out.extendedFrags.fastq -o out.rev.fastq ";
	...
	system "perl $Bin/rm_chimeric_seqs.pl -f split/seqs.fna -o seqs.fna -m $opts{O} -p";
	system "perl $Bin/reads_stats.pl -m $opts{m} seqs.fna ";
}
...
sub otu{
	...
	system "perl $Bin/txt_to_excel.pl -f  -s tab -r 2 -o 2.otu_table.xlsx otus/otu_table.txt";
	...
	system "perl $Bin/txt_to_excel.pl -s tab -f -r 2 -o 3.taxonomy.xlsx -n 'Phylum,Class,Order,Family,Genus,Species' taxa_plots/*_L?.txt ";
	system "perl $Bin/txt_to_excel.pl -s tab -f -r 2 -o 3.taxonomy_Group.xlsx -n 'Phylum,Class,Order,Family,Genus,Species' taxa_plots_Treatment/*_L?.txt ";
	system "perl $Bin/plot_16S_heatmap.pl -f 0.01  taxa_plots/taxa_summary_plots/";
}
...
sub venn{
	...
	my %fh;
	{
		...
		system "perl $Bin/plot_venn.pl -o venn/venn.png -t png venn/*.txt" if (scalar keys %has <6);
	}
}
...
sub alpha{
	...
	system "perl $Bin/collate_alpha.pl  alpha_diversity  collated/";
	system "perl $Bin/alpha_index_all.pl  otus/otu_table.biom  all_reads_diversity_index  ";
	system "perl $Bin/txt_to_excel.pl -f  -s tab -r 1 -o 4.alpha_diversity.xlsx  all_reads_diversity_index.txt collated/*.txt ";

	system "mkdir -p alpha_curves";
	system "perl $Bin/alpha_curves.pl --group $group  --output  alpha_curves/alpha_curves --type  $opts{t}  collated/ ";
...
sub beta{
	...
	if($opts{I}){
		...
		system "perl $Bin/plot_UPGMA_tree.pl cdout/bdiv_even$min/bray_curtis_dm.txt  cdout/bdiv_even$min/bray_curtis.pdf";
		system "perl $Bin/txt_to_excel.pl -s tab -f -r 1 -o 5.beta_diversity.xlsx cdout/bdiv_even$min/bray_curtis_dm.txt";
		system "mkdir -p 3d_plot";
		system "perl $Bin/plot_3d_plot.pl -m $map -o 3d_plot/ otus/otu_table.txt  " ;
		...
		system "perl $Bin/plot_UPGMA_tree.pl cdout/bdiv_even$min/weighted_unifrac_dm.txt  cdout/bdiv_even$min/weighted_unifrac_UPGMA.pdf" if $sample_num>3;
		system "perl $Bin/plot_UPGMA_tree.pl cdout/bdiv_even$min/unweighted_unifrac_dm.txt cdout/bdiv_even$min/unweighted_unifrac_UPGMA.pdf" ;
		system "perl $Bin/txt_to_excel.pl -s tab -f  -r 1 -o 5.beta_diversity.xlsx cdout/bdiv_even$min/*_dm.txt";
		system "mkdir -p 3d_plot";
		system "perl $Bin/plot_3d_plot.pl -m $map -o 3d_plot/ otus/otu_table.txt  " if $sample_num>3;
	}
}
